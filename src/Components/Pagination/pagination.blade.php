...
@if ($hasPaginator)
    <nav
        role="navigation"
        aria-label="{{ __('livewind::pagination.label') }}"
    ...
    >
        <p class="text-sm text-surface-foreground">
            @if ($firstItem && $lastItem)
                {{ __('livewind::pagination.showing') }} <span class="font-medium text-surface-foreground">{{ $firstItem }}</span>-<span class="font-medium text-surface-foreground">{{ $lastItem }}</span> {{ __('livewind::pagination.of') }} <span class="font-medium text-surface-foreground">{{ $total }}</span> {{ __('livewind::pagination.results') }}
            @else
                {{ __('livewind::pagination.total_results', ['total' => $total]) }}
            @endif
        </p>
        ...
        <div class="flex items-center gap-2">
            <button
                ...
                aria-label="{{ __('livewind::pagination.previous_aria') }}"
            >
                {{ __('livewind::pagination.previous') }}
            </button>
            ...
            @else
                <span class="px-2 text-sm text-surface-foreground">
                    {{ __('livewind::pagination.page_of', ['current' => $currentPage, 'last' => $lastPage]) }}
                </span>
            @endif
            ...
            <button
                ...
                aria-label="{{ __('livewind::pagination.next_aria') }}"
            >
                {{ __('livewind::pagination.next') }}
            </button>
        </div>
    </nav>
@endif
