...
it('renders associative options and wire model', function () {
    $html = Blade::render('<x-livewind::select model="category" label="Categoria" :options="[\'a\' => \'Alpha\', \'b\' => \'Beta\']" />');
...
it('renders array object style options', function () {
    $html = Blade::render('<x-livewind::select model="category" :options="[[\'value\' => \'a\', \'label\' => \'Alpha\']]" />');

    expect($html)->toContain('<option value="a">Alpha</option>');
});

it('renders custom option slot when options are empty', function () {
    $html = Blade::render('<x-livewind::select model="category"><option value="x">Custom</option></x-livewind::select>');

    expect($html)->toContain('<option value="x">Custom</option>');
});

it('merges consumer classes and attributes', function () {
    $html = Blade::render('<x-livewind::select model="category" class="w-64" data-test="select" />');
...
it('shows validation errors', function () {
    ...
    $html = Blade::render('<x-livewind::select model="category" label="Categoria" />');
